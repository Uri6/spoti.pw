...
- `DynamicBarCapabilities` inspects existing, loaded controllers/views without loading more views.
  ...
  layouts fail closed. These identities exist in the 9.1.78 binary; their real co-occurrence and
  lifecycle timing remain to be verified on a device.
...
| --- | --- |
| Ordinary audio | Attempts native compact hosting only when positive identities and measured layout fit. Real Spotify audio has not yet been validated. |
...
| Physical Spotify runtime | **Not tested: no connected iPhone available** |
